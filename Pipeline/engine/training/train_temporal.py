...
def derive_labels(customers, date_cutoff, prediction_days, min_prior_count, drop_threshold):
    cutoff = date_cutoff.replace(tzinfo=timezone.utc)
    pred_start = cutoff - timedelta(days=prediction_days)

    labels = {}
    for customer in customers:
        prior_count = sum(1 for t in customer.transactions if t.purchase_date < pred_start)
        recent_count = sum(1 for t in customer.transactions if t.purchase_date >= pred_start)
...
def main():
    ...
    labels = derive_labels(customers, date_cutoff, PREDICTION_DAYS, MIN_PRIOR_COUNT, DROP_THRESHOLD)
    ...
    observation_cutoff = date_cutoff.replace(tzinfo=timezone.utc) - timedelta(days=PREDICTION_DAYS)
